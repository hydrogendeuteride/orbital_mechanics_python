...
def partial_step(point1, point2, timestep):
    ret = point(0,0,0)
    ret.x = point1.x + point2.x * timestep
    ret.y = point1.y + point2.y * timestep
    ret.z = point1.z + point2.z * timestep
    return ret

class RK4_integrator:
    def __init__(self, timestep, bodies):
    ...
    def calculate_single_body_acceleration(self, body_index):
        ...
        for index, external_body in enumerate(self.bodies):
            if index != body_index:
                ...
                tmp_loc = partial_step(target_body.location, tmp_vel, 0.5 * self.timestep)
                ...
                tmp_loc = partial_step(target_body.location, tmp_vel, self.timestep)
    ...
    def update_location(self):
        for target_body in self.bodies:
            target_body.location.x += target_body.velocity.x * self.timestep
            target_body.location.y += target_body.velocity.y * self.timestep
            target_body.location.z += target_body.velocity.z * self.timestep
           
    def compute_velocity(self):
        for body_index, target_body in enumerate(self.bodies):
            acceleration = self.calculate_single_body_acceleration(body_index)
            target_body.velocity.x += acceleration.x * self.timestep
            target_body.velocity.y += acceleration.y * self.timestep
            target_body.velocity.z += acceleration.z * self.timestep 
...
if __name__ == "__main__":
    ...
    integrator = RK4_integrator(timestep = 10, bodies = bodies)
    motions = run_simulation(integrator, number_of_steps = 1e7, report_freq = 1e4)
